...
class CMoveMakeEngineeringNormal
	...
	def init(param)
		today = Time.new;
		@inPutPath = "#{$InPutPath}/MakeEngineeringNormal/#{param}/#{param}.json"
		@modelPath = "#{$ModelPath}/MakeEngineeringNormal/#{param}/#{param}.json"
		@configPath = "#{$ModelPath}/MakeEngineeringNormal/#{param}/config.json"
	...
	def step3_directCopy(param)
		...
		param.each do |list|
			...
			if list['except'] and list['except']!=""
				@ObjThink.start({"action" => "WriteCopyFile","sourcePath" => @Config['enginneringPath']+list['sourcePath'],"targetPath"=>@Config['generatePath']+list['targetPath'],"except"=>list['except'],"contain"=>contain})
			else
				@ObjThink.start({"action" => "WriteCopyFile","sourcePath" => @Config['enginneringPath']+list['sourcePath'],"targetPath"=>@Config['generatePath']+list['targetPath'],"contain"=>contain})
			end
		end
	end
	...
	def step3_directMerger(param)
		...
		param.each do |list|
			...
			@ObjThink.start({"action" => "WriteFile","path" => @Config['generatePath']+list['targetPath'],"isCover"=>true,"text"=>targetText})
		end
		...
		param.each do |list|
			...
			selectorFileList.each do |list_1|
				...
				sourceList.each do |key,value|
					...
					@ObjThink.start({"action" => "WriteFile","path" => @Config['generatePath']+targetFile,"isCover"=>true,"text"=>controlText})
	...
	def step3_replaceMerger(param)
		...
		param.each do |list|
			...
			selectorFileList.each do |list_1|
				...
				targetFile = targetFile.sub(@Config["targetFileName"],selectorList[i])
				puts "	"+@Config['generatePath']+targetFile
				@ObjThink.start({"action" => "WriteFile","path" => @Config['generatePath']+targetFile,"isCover"=>true,"text"=>replaceTarget})
				i=i+1
			end
